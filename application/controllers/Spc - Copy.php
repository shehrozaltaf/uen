...
class Spc extends MX_Controller
{
	...
	function ml_dashboard(){
		...
		if($this->users->in_group('admin') || $this->users->in_group('management')){
			...
			from clusters where cluster_no not like '%501' and cluster_no not like '%502' and cluster_no != 'null' group by dist_id order by dist_id");
			...
			if(!empty($this->uri->segment(3))){
				...
				if($type == 'rc'){
					...
					(select count(distinct rndid) from forms where SUBSTRING(cluster_code, 1, 3) = l.enumcode and cluster_code = l.hh02 and istatus > 0 and istatus < 96 and istatus is not null and istatus != '' and istatus != 'null') as collected_households,
					...
					where l.enumcode = '$d' and l.hh02 not like '%501' and l.hh02 not like '%502' and l.hh02 != 'null' and randomized = '1'
					...
					where l.enumcode = '$d' and l.hh02 not like '%501' and l.hh02 not like '%502' and l.hh02 != 'null' and randomized = '1'
					and l.username not in('afg12345','user0001','user0113','user0123','user0211','user0234','user0252','user0414','user0432', 'user0434')
					and (select count(*) from ml_randomised where dist_id = l.enumcode and hh02 = l.hh02) = (select count(distinct rndid) from forms where SUBSTRING(cluster_code, 1, 3) = l.enumcode and cluster_code = l.hh02 and istatus > 0 and istatus < 96 and istatus is not null and istatus != '' and istatus != 'null')
					group by l.enumcode, l.hh02
					order by l.enumcode,l.hh02");
				...
				from clusters c
				left join listings l on l.hh02 = c.cluster_no
				where l.hh02 not like '%501' and l.hh02 not like '%502' and l.hh02 != 'null' and randomized = '1'
				and l.username not in('afg12345','user0001','user0113','user0123','user0211','user0234','user0252','user0414','user0432', 'user0434')
				group by l.enumcode, l.hh02
				order by l.enumcode,l.hh02");
			} 
			...
			$this->data['clusters_by_district'] = $this->spc->query("select dist_id, 
			count(*) clusters_by_district from clusters where cluster_no not like '%501' and cluster_no not like '%502' and dist_id = '$district'
			group by dist_id order by dist_id");


			$this->data['randomized_clusters'] = $this->spc->query("select dist_id,
			sum(case when randomized = '1' then 1 else 0 end) randomized_c
			from clusters where cluster_no not like '%501' and cluster_no not like '%502' and cluster_no != 'null' and dist_id = '$district' 
			group by dist_id order by dist_id");
			...
			(select count(distinct hhno) from forms where dist_id = c.dist_id and cluster_code = c.cluster_no and cast(istatus as int) > 0 and cast(istatus as int) < 96 and istatus is not null and istatus != '' and istatus != 'null') as hh_collected
			from clusters c
			where c.cluster_no not like '%501' and c.cluster_no not like '%502' and c.cluster_no != 'null' and c.randomized = '1'  and c.dist_id = '$district'
			group by c.dist_id, c.cluster_no order by c.dist_id");
			...
			from clusters c
			left join listings l on l.hh02 = c.cluster_no
			where l.enumcode = '$district' and l.hh02 not like '%501' and l.hh02 not like '%502' and l.hh02 != 'null' and randomized = '1'
			and l.username not in('afg12345','user0001','user0113','user0123','user0211','user0234','user0252','user0414','user0432', 'user0434')
			group by l.enumcode, l.hh02
			order by l.enumcode,l.hh02");
	...
	function index(){
		...
		if($this->users->in_group('admin') || $this->users->in_group('management')){
			...
			$this->data['clusters_by_district'] = $this->spc->query("select (case when dist_id = 113 then 'Rahim Yar Khan'
			when dist_id = 123 then 'Muzaffargarh'
			when dist_id = 211 then 'Badin'
			when dist_id = 234 then 'Qambar Shahdadkot'
			when dist_id = 252 then 'Sanghar'
			when dist_id = 414 then 'Lasbela'
			when dist_id = 432 then 'Jaffarabad'
			else 'Naseerabad' end) as district, 
			count(*) clusters_by_district from clusters where cluster_no not like '%501' and cluster_no not like '%502'
			group by dist_id order by dist_id");
	...
	function randomized_households(){
		...
		$cluster = $this->uri->segment(3);
		$this->data['get_list'] = $this->spc->query("select sno, concat(tabNO, '-', substring(compid, 8, 8)) as hh_no from ml_randomised where hh02 = '$cluster' order by cast(sno as int)");
	...
	*/
	...
	function update_data(){
		...
		$data = $this->spc->query("select rndid
		from forms where len(hhno) = 8 
		and username not in('afg12345','user0001','user0113','user0123','user0211','user0234','user0252','user0414','user0432', 'user0434')
		and rndid != 186
		order by rndid");
		...
		foreach($data->result() as $row){
			...
			$hhno = $this->spc->query("select hhno from ml_randomised where _id = $row->rndid")->row()->hhno;
			//$this->spc->query("update forms set hhno = '$hhno' where rndid = $row->rndid");
		}
